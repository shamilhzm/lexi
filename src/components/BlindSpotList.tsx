  ...
  if (stats.length === 0) {
    return (
      <div className="bg-panel border border-line rounded-[12px] px-6 py-8 text-center">
        ...
        <h3 className="text-[15px] font-bold mb-1">No blind spots yet</h3>
        <p className="text-dim text-[13px]">Do some drills — every miss is tracked here so you can target your weak points.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-panel border border-line rounded-[12px] p-4">
      <div className="flex items-baseline gap-2 mb-3">
        <span className="font-mono font-bold text-[20px] tabular-nums">{total}</span>
        <span className="text-dim text-[13px]">misses across {stats.length} area{stats.length === 1 ? '' : 's'} · last {days} days</span>
      </div>
      ...
      <button onClick={() => onDrill()}
        className="mt-4 flex items-center gap-2 bg-amber text-bg font-bold rounded-[10px] px-4 py-2 text-[13px] hover:brightness-105">
