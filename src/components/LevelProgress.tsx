...
export default function LevelProgress() {
  ...
  const canAdvance = edge && edgeIdx < ALL_LEVELS.length - 1 && edgeStat && edgeStat.coverage >= ADVANCE;
    ...
    <div className="bg-panel border border-line rounded-[12px] p-3 sm:p-4 mb-4">
      <div className="flex items-center justify-between mb-2.5">
        ...
        {canAdvance && (
          <button onClick={() => focusUpTo(edgeIdx + 1)}
            ...
            {edge} is {Math.round(edgeStat!.coverage * 100)}% — add {ALL_LEVELS[edgeIdx + 1]} <ChevronRight size={12} />
          </button>
        )}
      </div>
      <div className="flex gap-1.5">
        {stats.map((s, i) => {
          ...
          return (
            <button key={s.level} onClick={() => focusUpTo(i)} title={`${s.learned}/${s.count} learned`}
              ...
              <div className="h-1.5 rounded-full bg-bg mt-1.5 overflow-hidden">
                <div className="h-full" style={{ width: `${Math.max(pct, 2)}%`, background: heat(s.coverage) }} />
              </div>
