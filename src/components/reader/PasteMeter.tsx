// "Can I read this?" — the comprehension meter on text the learner brings.
//
// Moved out of views/Read.tsx on 2026-09-24 when Lesen became the news reader;
// the reasoning below is unchanged and still the reason it reads this way.
//
...
import { coverageOf, unlocksToReach, ASSISTED, INDEPENDENT, type Coverage } from '../../lib/coverage.ts';
import { stateOf } from '../../lib/learnerState.ts';
import { savedTexts, saveText, removeText } from '../../store.ts';
...
export default function PasteMeter({ onStudy }: { onStudy: (t: Target) => void }) {
  const v = useStore();
  ...
  return (
    <section aria-labelledby="lesen-meter" className="flex flex-col gap-4">
