    ...
    <div className="text-center bg-panel border border-line rounded-2xl px-8 sm:px-10 py-12 max-w-md w-full">
      ...
      {data.milestone && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          <Trophy size={15} /> <span className="font-semibold text-[0.8125rem]">New milestone · {data.milestone}</span>
        </motion.div>
      )}
      {tiles.length > 0 && (
        <div className="grid divide-x divide-[var(--color-line)] border border-line rounded-[10px] mb-6"
          ...
          {tiles.map((s) => (
            <div key={s.label} className="px-2 py-3">
              <div className="text-[0.6875rem] text-dim uppercase tracking-[1px]">{s.label}</div>
              {/* Count-up: feedback density, honors reduced motion via CountUp. */}
              <div className={`font-mono font-bold text-[1.25rem] mt-0.5 tabular-nums ${s.tone}`}><CountUp value={s.num} from={0} suffix={s.suffix ?? ''} /></div>
            </div>
          ))}
        </div>
      )}
      {data.minedCount !== undefined && data.minedCount > 0 && (
        <p className="text-[0.8125rem] text-dim mb-5">{data.minedCount} of today’s words came from your own texts.</p>
      )}
      {children}
    </div>
  );
