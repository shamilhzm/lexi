 ...
 *  failed to know. Decided from the separators touching the token: a dot, `@` or
 *  slash with no space, between two word tokens. */
const ADDRESS_SEP = /^[.@/:]$/;
...
export interface CoverageOptions {
   ...
   *  `lib/wiktionary.ts`). Excluded like the structural entities above, but only
   *  when the corpus does not resolve the token: a name that is also a word the
   *  learner has (*Koch*) stays counted. The capitalised-and-unresolvable rule
   *  this replaces for real text was rejected for sweeping up nouns; a dictionary
   *  saying "Nachname" is evidence, a capital letter is not. */
