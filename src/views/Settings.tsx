  ...
  return (
    <div className="max-w-[640px] mx-auto">
      ...
      <section className="bg-panel border border-line rounded-md p-4 sm:p-5 mb-4">
        <div className="flex items-center gap-2 mb-1"><Palette size={16} className="text-amber" /><h2 className="text-base font-semibold">Appearance</h2></div>
        ...
        <div className="flex flex-wrap gap-2">
          {THEMES.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => pickTheme(id)}
              className={`flex items-center gap-2 text-xs rounded-md px-3.5 py-2 border transition-colors ${theme === id ? 'border-amber text-amber bg-panel2' : 'border-line text-dim hover:border-amber'}`}>
              <Icon size={15} /> {label}
            </button>
          ))}
        </div>
      ...
      <section className="bg-panel border border-line rounded-md p-4 sm:p-5 mb-4">
        <div className="flex items-center gap-2 mb-1"><Type size={16} className="text-amber" /><h2 className="text-base font-semibold">Text size</h2></div>
        ...
        <div className="flex flex-wrap gap-2">
          {[{ v: 0.875, label: 'Compact' }, { v: 1, label: 'Standard' }, { v: 1.125, label: 'Large' }, { v: 1.25, label: 'Larger' }].map(({ v, label }) => (
            <button key={v} onClick={() => setTextScale(v)}
              className={`text-xs rounded-md px-3.5 py-2 border transition-colors ${textScale() === v ? 'border-amber text-amber bg-panel2' : 'border-line text-dim hover:border-amber'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <section className="bg-panel border border-line rounded-md p-4 sm:p-5 mb-4">
        <div className="flex items-center gap-2 mb-1"><Gauge size={16} className="text-amber" /><h2 className="text-base font-semibold">Review intensity</h2></div>
      ...
      {/* HD voice */}
      <section className="bg-panel border border-line rounded-md p-4 sm:p-5 mb-4">
        <div className="flex items-center gap-2 mb-1"><Volume2 size={16} className="text-amber" /><h2 className="text-base font-semibold">German voice</h2></div>
        <p className="text-dim text-xs mb-3">
        ...
        {hdVoice() ? (
          ...
          <button onClick={enableHd} className="flex items-center gap-2 bg-amber text-bg font-bold rounded-md px-4 py-2.5 text-base hover:brightness-105">
      ...
      <section className="bg-panel border border-line rounded-md p-4 sm:p-5 mt-4">
        <div className="flex items-center gap-2 mb-1"><Archive size={16} className="text-amber" /><h2 className="text-base font-semibold">Your data</h2></div>
        <p className="text-dim text-xs mb-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <button onClick={doExport} className="flex items-center gap-2 bg-amber text-bg font-bold rounded-md px-4 py-2.5 text-base hover:brightness-105">
            <Download size={15} /> Export backup
          </button>
          <button onClick={() => fileRef.current?.click()} className="flex items-center gap-2 bg-panel2 border border-line rounded-md px-4 py-2.5 text-xs hover:border-amber">
