...
export default function Settings() {
  ...
  return (
    <div className="max-w-[640px] mx-auto">
      ...
      {/* HD voice */}
      <section className="bg-panel border border-line rounded-[12px] p-4 sm:p-5 mb-4">
        ...
        <p className="text-dim text-[12.5px] mb-3">
        ...
        {hdVoice() ? (
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button onClick={() => setHdVoice(false)} className="text-[13px] text-dim hover:text-red ml-auto">Turn off</button>
          </div>
        ) : dl !== null ? (
          <div className="flex items-center gap-2 text-[13px] text-dim"><Loader2 size={15} className="animate-spin" /> Downloading voice… {dl}%</div>
        ) : (
          <button onClick={enableHd} className="flex items-center gap-2 bg-amber text-bg font-bold rounded-[10px] px-4 py-2.5 text-[14px] hover:brightness-105">
            <Download size={15} /> Enable HD German voice
          </button>
        )}
        {hdErr && <p className="text-red text-[12.5px] mt-2">{hdErr}</p>}
      </section>

      {/* AI provider */}
      <section className="bg-panel border border-line rounded-[12px] p-4 sm:p-5">
        ...
        <label className="text-[12px] text-dim block mb-1">Preset</label>
        <div className="flex flex-wrap gap-2 mb-3">
          {Object.keys(PRESETS).map((name) => (
            <button key={name} onClick={() => applyPreset(name)}
              className={`text-[12.5px] rounded-full px-3 py-1.5 border transition-colors ${base === PRESETS[name].baseUrl ? 'border-amber text-amber' : 'border-line text-dim hover:border-amber'}`}>
              {name}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <button onClick={saveAi} className="bg-amber text-bg font-bold rounded-[10px] px-5 py-2.5 text-[14px] hover:brightness-105">Save</button>
...
function Field({ label, value, onChange, placeholder, mono, password }:
  ...
  return (
    <div className="mb-3">
      <label className="text-[12px] text-dim block mb-1">{label}</label>
