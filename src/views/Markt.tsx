      ...
      <div className="bg-panel border border-line rounded-[10px]">
        <div className="flex items-center gap-2.5 px-3 sm:px-4 py-3 border-b border-line flex-wrap">
          {zoom ? (
            <>
              ...
              <h2 className="text-[13px] sm:text-[15px] font-semibold truncate max-w-[38vw] sm:max-w-none">{zoom}</h2>
            </>
          ) : (
            <>
          ...
          <div className="flex items-center gap-2 ml-auto flex-wrap">
            ...
            <LevelFilter />
            {zoom
              ? <button onClick={() => onStudyGroup(zoom)} className="flex items-center gap-1.5 bg-amber text-bg font-bold rounded-md px-3 py-1.5 text-[13px] hover:brightness-105"><Play size={13} /> Study {shortName(zoom)}</button>
              : <button onClick={onStudyAll} className="flex items-center gap-1.5 bg-amber text-bg font-bold rounded-md px-3 py-1.5 text-[13px] hover:brightness-105"><Play size={13} /> Study all</button>}
          </div>
        </div>
        ...
        {list ? (
          ...
          <div ref={boxRef} className="relative w-full" style={{ height: 'min(60vh, 580px)' }}>
            ...
            {tiles.length === 0 && (
              <div className="absolute inset-0 grid place-items-center text-dim text-[13px] px-6 text-center">No sectors at the selected CEFR levels — widen the filter.</div>
            )}
            {tiles.map((t, idx) => {
            ...
            {hover && (
              <div className="fixed z-50 pointer-events-none bg-card border border-amber rounded-lg px-3 py-2.5 text-[13px] shadow-2xl"
                ...
                <h4 className="text-[13px] font-semibold mb-1.5">{hover.c.name}</h4>
  ...
  return (
    <div className="flex rounded-md border border-line overflow-hidden">
      <button onClick={() => onChange(false)} title="Treemap" aria-pressed={!list}
        className={`flex items-center gap-1 px-2 py-1.5 text-[12px] ${!list ? 'bg-panel2 text-amber' : 'text-dim hover:text-txt'}`}><LayoutGrid size={13} /> <span className="hidden sm:inline">Markt</span></button>
      <button onClick={() => onChange(true)} title="List" aria-pressed={list}
        className={`flex items-center gap-1 px-2 py-1.5 text-[12px] border-l border-line ${list ? 'bg-panel2 text-amber' : 'text-dim hover:text-txt'}`}><List size={13} /> <span className="hidden sm:inline">Liste</span></button>
    </div>
  );
}
...
function ListView({ cells, zoom, onTap, onStudy }: { cells: Cell[]; zoom: boolean; onTap: (c: Cell) => void; onStudy: (c: Cell) => void }) {
  if (cells.length === 0) {
    return <div className="grid place-items-center py-16 text-dim text-[13px] px-6 text-center">No sectors at the selected CEFR levels — widen the filter.</div>;
  }
  return (
          ...
          <div key={c.name} className="flex items-center gap-3 px-3 sm:px-4 py-2.5 hover:bg-panel2">
            <button onClick={() => onTap(c)} className="flex-1 min-w-0 text-left" title={zoom ? 'Study sector' : 'Open sectors'}>
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[11px] text-dim font-mono flex-shrink-0">{c.sub}</span>
              </div>
            ...
            <span className="font-mono font-bold text-[15px] tabular-nums w-12 text-right" style={{ color: heat(p) }}>{Math.round(p * 100)}%</span>
