...
// The attested inflection table, as the app loads it — without it this measures a
// weaker matcher than the one a learner reads with.
const inflPath = join('public', 'data', 'inflections.json');
const attested = existsSync(inflPath) ? JSON.parse(readFileSync(inflPath, 'utf8')) as Record<string, string[]> : null;
const m = buildMatcher(corpus, attested);
