...
| A6 | *"This browser has no notification support"* on iOS (P54) | Safari supports Web Push for Home-Screen-installed apps. Detect standalone; when not installed, offer the install step instead of declaring the browser incapable. **Finished 2026-09-07 on the phone:** the new copy was landing at `disabled:opacity-60`, because on iOS Safari `perm` is `unsupported` — the same branch as a browser that genuinely cannot do this — so the row said *Share → Add to Home Screen* in the grey of an unavailable control. That is the removed sentence, said in styling. The install advice is not a button now: there is nothing to press (the action is in Safari's share sheet) and `<button disabled>` is also what a screen reader announces as unavailable | An iPhone user is told what to do, not what cannot be done — ✅ verified on a 16 Pro, at full contrast beside *Add to calendar* |
...
| B4 | ✅ **shipped 2026-09-08.** The installed PWA waits on the network before showing anything (P41, P53) | **The fallback only fires when `fetch` *rejects*, and that is the whole defect.** Both handlers were network-first with a cache fallback — textbook, and fine on a dead connection, which rejects fast. A *slow* one does not reject; it hangs, and an installed app holding every byte it needs sat on a white screen waiting for permission to show them. Everything is served from the cache first now and checked afterwards. The write went the same way: `/data/` re-`put` **5.79 MB decoded** into Cache Storage on every launch (zero wire bytes — the HTTP cache answered — but the copy happened anyway), and the server already answers a conditional request with **304, 0 bytes**, so the worker compares the ETag it got back against the one it is holding and skips the `put` when they match. `CACHE` is deliberately *not* bumped: this is a change of strategy, not of content, and a bump would delete up to 19 MB of shards a learner has accumulated. The cost — the launch after a deploy paints the previous build — is closed by `reloadIfBuildMoved`, which compares the *build stamp* (stable; an ETag is not, since content negotiation can give identical bytes two ETags) and reloads once, guarded three ways so it can never loop | ✅ **Server stopped, cold navigation to `#/words` on a 16 Pro: the full Themen surface renders.** In the pane, warm: navigation `responseEnd` **11 ms**, `loadEnd` **37 ms**, every request `deliveryType: cache-storage` at **0 bytes**, total transfer 355 B (the build stamp). Was: navigation on the network at 8,156 B, `/data/` revalidating through the HTTP cache. Guarded in `sw.test.ts` — the worker is driven in a fake scope, and the strategy it replaced is run beside it against a `fetch` that never settles, because a guard nobody has seen fail is not a guard |
| B5 | 🟡 **partly shipped 2026-09-08** — 1,603 → **918 kB**, target not met. Cold boot is too big (P50) | `detail.json` was **857 kB gz** — 54% of a first visit — fetched whole by everyone, always. It is six files now, one per CEFR level (A1 172 / A2 183 / B1 275 / B2 111 / C1 76 / C2 25), and the surfaces ask for what they are about to show. The level split is refused for `cards.json` and safe here for the same reason detail could be deferred at all: no count, completion or snapshot reads `ex`/`def`/`defDe`, and detail is already optional by contract. **The first attempt asked for the whole rendered page and that was 24 slots, which unplaced spans A1+A2+B1 — three shards, 636 kB, before the learner had scrolled past word one.** So the feed follows the thumb: the dwell observer already knows how far down the learner has got, and detail loads for that slot plus `LOOKAHEAD`. Üben waits on `levels()` rather than on its queue, because the queue cannot be built without `ex` — placement narrows that to A1..your level | 🟡 **Measured on an iPhone 17 Pro against `npm run measure`, which counts at the wire because the browser pane cannot** (it runs hidden, so IntersectionObserver never fires, and it keeps an HTTP cache that hides the second measurement of a cold boot). A cold feed launch: **918 kB, one shard** — cards 294, JS 215, detail/A1 172, inflections 95, fonts 95, freq 17, css 12, rest 18. Was 1,603. Opening Üben unplaced pulls the other five (+670 kB); after placement at A1 it pulls nothing. **Under 700 kB is not reachable this way** — the boot core without any detail is already 639 kB. What is left, measured: `cards.json` 294, the JS bundle 215, fonts 95 (fraunces-var alone is 66, and there is no font tooling in the repo to check whether it is subset). Those are three separate pieces of work |
...
| # | What | Do | Done when |
|---|---|---|---|
| C1 | ✅ **shipped 2026-09-08.** The feed has **zero headings** and 106 focusables (P23, P26) | Re-measured on the live build first, and it was worse than filed: **one** heading, which was the welcome slot's `h1` and therefore only present for a *cold* learner — a returning one got none at all, on the app's front door. Now: an `sr-only` `h1` naming the surface, every word slot an `h2` carrying the headword, and every slot a `section` named by `aria-labelledby` pointing at that same heading, so the region and its name cannot drift and the article comes with it (*die Reise*, not *Reise*). The welcome and end slots are `h2` siblings for the same reason — two `h1`s is two answers to *where am I* | ✅ Measured on the built app: **26 headings and 25 named regions** on the feed, against 1 and 0. The 111 focusables are unchanged and were never the defect — 24 words × 3 actions is the right number of controls; having no way past them was the defect |
| C2 | ✅ **shipped 2026-09-08.** 24 empty `aria-live="polite"` regions on the feed (P24) | One per slot, announcing the save confirmation — so a feed of 6,700 words would have carried 6,700. A live region is a standing instruction to interrupt whatever is being read; the right number for a surface is one, and an empty one is cost with no message. There is one now, owned by the feed. The per-slot line stays as *visible* text and is `aria-hidden`: the bookmark's own `aria-pressed` is how the state is exposed, and the announcer covers the change. Keyed on a counter, not the text, and the region is mounted from first render — an `aria-live` element has to be in the tree before its content changes or the change is not a change | ✅ **1 live region on the feed** (was 24); at most one on every surface |
| C3 | ✅ **shipped 2026-09-08.** Bar labels fall under AA over their own backdrop (P42, P62) | **The ink moved, not the material.** `--color-dim` is chosen to clear AA on paper and on white, which is the right test for prose on a page and the wrong one for a label on a bar whose ground is whatever is sliding underneath — measured 4.27 light / 3.72 dark over a `txt` headword at the engaged alpha. Raising the fill needs ~90%, which is a panel wearing a blur; a plate behind every label is four rectangles in a bar whose job is not to be a rectangle. So `.glass-bar` overrides `--color-dim` to a new `--color-chrome` — `dim` nudged one step toward `txt`, `#55544e` / `#a8b7bd` — which every `text-dim` inside a bar picks up with no caller having to remember. The active tab's pill went opaque so the accent is read against the pill (4.40 → 4.62 light, 8.13 dark) | ✅ **The pin is deleted and the guard is real.** Worst case over the same saturated grounds is now **4.63 light / 4.61 dark**, asserted at ≥ 4.5, plus a guard that chrome stays quieter than `txt` so the fix has not flattened the hierarchy it was protecting. Fails on the injected old value |
| C4 | 🟡 **half shipped 2026-09-08** — the mispronunciation is fixed, the non-audio cue is not. Without audio, IPA is the whole pronunciation story (P27, P20) | `speakDe` set `lang = 'de-DE'`, looked for a German voice, and **spoke anyway when there wasn't one** — an utterance with no voice is read by the device default, so an English phone applied English phonology to German spelling (*Zeit* as "zyte", every `ei` wrong) and said nothing about it. It refuses now and returns why; `de-AT` and `de-CH` count as German, because a learner with Austrian German installed is far better served by it than by an American voice. The speaker is not *offered* when it cannot work: `useGermanVoice()` subscribes to `voiceschanged` (the list is empty on first read in every browser that fires it) and the button becomes a muted `VolumeX` with the reason as its label — told before pressing, not met with silence after | 🟡 **A hearing learner is never mispronounced at silently — done**, guarded in `speak.test.ts`, which fails if an English voice is ever handed German. **The non-audio cue is not done and should not be faked here:** a respelling is a corpus fact, and CLAUDE.md's authoring rule is that facts are never generated. The tractable version — stress position derived from the `ˈ` already in the IPA, stated in words — is a `scripts/corpus/*` pass and belongs in Track D |
...
| # | What | Do | Done when |
|---|---|---|---|
| D1 | ✅ **shipped 2026-09-08 — 5/24 → 24/24.** The determiner hole (P38) | 17 cards authored through the gate in two batches, every one verified against de.wiktionary for gender, plural, part of speech and IPA, and every example proved to contain a real inflection by the app's own matcher. `kein` (rank 170), `nichts`, `etwas`, `jeder`, `welcher`, `andere`, `beide`, `jemand`, `einige`, `mehrere`, `solche`, `manche`, `selber`, `gesamt`, `irgendein`, `sonstige`, `jegliche`. **The gate was never the blocker** — the standing theory was that `POS_MAP` has no `det`, but 11 of the first 14 verified on the first run; the three rejections were a wrong `pos` on two number-words and an example the matcher could not match. It had simply never been authored. One measurement fix went with it: `ander` is rank 42 and is a *lemmatiser stem*, not a word anybody writes, so a card for `andere` left the row reading untaught — `STEM_FORMS` names the four such rows by hand, for the same reason `MEANINGFUL_FUNCTION` is written out by hand | ✅ `corpus:kernwortschatz` reports **24/24**, against a target of ≥20 |
| D2 | 🟡 **partly done 2026-09-08 — the queue was 4/5 instrument, and sharpening it was worth more than adjudicating the noise.** The gloss-vote reading order (P32) | Two artefact classes were removed before reading. The stemmer trimmed a bare `es`, taking *strawberries* to *strawberrie*, which neither prefixes nor is prefixed by *strawberry* — so `die Erdbeere`, glossed *strawberry*, defined as a red fruit and exampled with *Strawberries are red*, was reported as a card disagreeing with itself. And a gloss made entirely of stop words (*to get used to*, *form*) arrived at the vote with an empty word list, could agree with nothing by construction, and lost every time. **105 → 88**, and the 17 that went were all good cards. Of what remains, the gloss defects are rarer than the *definition* defects the file's own header already names (`man` defined *just; only*, `zu` defined *too*, `auf` defined *finished; gone*) | 🟡 Read to the end of the A1–B1 band. One real gloss fixed: **`die Bahn`** was *route, trail* on an A1 card whose both examples are about trains — now *train, railway; track, lane*. The rest of the band is paraphrase, which is what a good definition does. **The pass's real find was elsewhere** — see the spelling row below |
| D3 | ✅ **shipped 2026-09-08 — both columns within 10.** The register bias (P17, P18) | **A third of the reported gap was accounting, and finding that first stopped a duplicate being authored.** Three measurement defects, each the same shape — a reference row whose lexeme *is* taught under a different surface form: (a) rows carrying a single inflected form, so the Kinder list's `keine`, `jede`, `viele`, `wenige` counted as owed while `kein`, `jeder`, `viel`, `wenig` are cards; (b) a lemmatiser stem, `letzt` against the taught `letzte`; and (c) **government — `fold` did not strip `auf + A`, so all 47 governed cards were invisible**, which reported `achten` as owed and nearly had it written a second time (the corpus validator caught the collision). Fixing those took the queues from 82/79 to 79/71 before a single card was written. Then 65 cards authored through the gate. `--missing` also stopped defaulting to `gesamt` on a corpus name it does not know: `--missing foren` returned a full, plausible, wrong queue labelled *Gesamtkorpus*, which cost a measurement | ✅ **Zeitungen 32 · Foren 39 · Kinder 42** — gaps of **7** and **10** against a target of 10 (was 38 and 35). 🟡 **One thing the maintainer should see, not me:** 25% of what remains in the Foren top-1000 queue is nationality, religion and insult vocabulary — *Türke*, *Jude*, *Christ*, *Nazi*, *Arsch*, *doof* — because the Foren corpus is internet-forum German and skews to argument. The Kinder queue is 0% that. Closing the Foren column the rest of the way means deciding to teach those words; I have not made that decision |
| D4 | 🟡 **not finishable in a sitting, and now measured rather than estimated.** Phase 1 of DICTIONARY.md (P61) | **The blocker was the queue, not the writing.** `corpus:candidates` ranks on the news count, and Wiktionary keeps a common-noun homograph for every German surname — so its top thirty offered `Müller` (*miller*), `Fischer`, `Wagner` (*cartwright*), `Kiel` (*keel*), and `Johannes` and `Jürgen` glossed as genital slang, as top-twenty vocabulary. `isLikelyEntity` cannot see them: it needs two capitals and German capitalises every noun. Four filters added, each dropping a *class*: also filed as a name (320 forms), demonyms (`-er` on any place name), glosses that describe a spelling rather than a meaning (`new`, `Merz`, `Corona`, `dar`), and lemmas already taught under a government (`teilnehmen an + D`, `sich konzentrieren auf + A` — both were written into batch 6 and refused by the gate before the cause was clear) | 🟡 **88.9% of running text, against a target of 95%.** Batch 6 shipped 28 cards and moved it **+0.1 pp**, so 95% is ~**1,700 more cards** — about 43 batches at the realistic size, which `DICTIONARY.md` now records as ~30 rather than ~200. The old estimate of "~2,800 cards" was never measured; this one is. The queue is usable now, which is the part that was actually stopping the phase |
| D5 | 🟡 **mostly shipped 2026-09-08 — 52/90 → 86/90 taught.** Migration vocabulary (P22) | 38 cards authored through the gate across three batches: the whole Trade & site set (16/16), the nursing and medicine vocabulary a care worker uses on shift, `der Aufenthaltstitel`, `die Ausländerbehörde`, `die Lohnabrechnung`, `der Betriebsrat`, `das Elterngeld`, `die Steuernummer`, `die Einschulung`, and the school and family words a parent meets in the first term. **Found on the way: the sector taxonomy has 274 sectors and not one for the trades** — `Schraube`, `Zange`, `Gerüst` and `Schaltplan` are filed under *Objects & Materials*, *The Building* and *Technology & Devices* because there is nowhere better, which is its own small finding about who the corpus was built for | 🟡 **≥75 taught: done, 86/90 (95.6%).** 0 absent: **not** done, and the two left are honest refusals rather than omissions. `Meldebescheinigung` has no de.wiktionary entry, so the gate will not vouch for it and must not be talked round — that is the rule working. `Termin ausmachen` is a collocation, not a headword, and belongs to whatever answers the phrase question. Both are named here rather than quietly dropped |
...
### Track E — ✅ **ruled and shipped 2026-09-09**

*All six are decided and written into [VISION.md](VISION.md) as "Six questions,
ruled". Every premise was measured before it was argued and **three were already
false**; the rulings and the properties they produce are pinned in
`src/rulings.test.ts`, so a decision walked back by accident fails a test.*

| # | Ruling | What shipped, or why nothing did |
|---|---|---|
| E1 | **Ordered, not composed — and the ordering was made to exist** | The filed premise ("twenty verbs") was stale, and straight rank gave something worse: `sie · auf · sein · aus · auch · stehen · so · sollen · sagen · er`, **no noun until 22**, `eligibleModes` empty for the whole opening session. Two fixes, neither of which names a card. (a) `corpus:freq` now projects the **Kernwortschatz** reference instead of `provenance.json` — **80.0% of the corpus and 86.3% of A1**, up from 29% and 7.4%, with `sein` at 4 and `grillen` at 8,451; the 171 demonym/newswire cards the reference does not know keep their old rank *behind* it. (b) `firstRunIds` reserves a third of the opening for cards the app can drill, filled by rank — `das Ende` (25), `die Arbeit` (69), `der Tag` (98) — preferring a gender the session does not have yet, because rank alone reserved three `das` nouns. First shipping use of that reference, so ATTRIBUTIONS §6 changes from *"what ships: nothing"* |
| E2 | **No — a wrong answer is not explained** | 434 of 1,174 verbs decompose onto another card, and it splits: **216 separable** (`anrufen` = an- + `rufen`) against **167 inseparable**, where the same rule says something false (`bekommen` ≠ be- + `kommen`, `erzählen` ≠ er- + `zählen`). It is also a rule of the language, and generating it would break commitment 5. Filed, not decided: *linking* the base card, which claims nothing |
| E3 | **No change to distractors** | The objection is false and measured: over 2,282 cards, pick-the-longest scores **24.4%** against **25.0%** chance, most-words 16.1%, shortest 20.7%. Deliberately near-miss distractors stay declined on the weaker evidence — the drill they would fix is not broken |
| E4 | **Typed production is what this app can grade** | `recall` stays; speech scoring stays refused under commitment 3. The limit is stated, not closed |
| E5 | **Yes — and a signal the scheduler acts on is a signal it names** | It failed twice: `Briefing.weakSectors` was computed and read by nothing, and every fresh card arrived as a bare `fresh` that `whyLine` was silent on. `SessionReason` now carries *which* fresh, and the card says **"You saved this one"** or **"You kept stopping on this in the feed"** — an observation, never an inference about knowing or wanting. Verified end to end on the running app |
| E6 | **Say plainly that Lexi does not answer it** | `PathCard` ends with one line: these letters count words studied here, they are not an exam result, and Goethe's and telc's free *Modellsätze* are what can tell you. Silence was the worse option — the app shows a CEFR letter and a lit-up path, so it has made the claim either way |
...
| F2 | ✅ **done 2026-09-09.** The feed opens on a lottery (P55) | Two halves, a day apart. The sort by CEFR band then core rank landed first and could not work: `freq.json` ranked **87 of 1,170 A1 cards** and none of `sein`, `haben`, `werden`, `gehen`, `Zeit`, `gut`, so the commonest words in the language fell into a tail ordered by build position and `grillen` was still introduced 70th against `sein` at 118th. `corpus:freq` now projects the Kernwortschatz reference — **80.0% of the corpus, 86.3% of A1** — and `firstRunIds` keeps a third of the opening for cards the app can actually drill. See Track E / VISION decision 5 | S |
...
5. ~~**Open E1 and E6 in VISION** as written questions before anyone writes code
   against them.~~ ✅ **done 2026-09-09, and then ruled — all six.** Each premise was
   measured before it was argued and three were false. Two of the six turned out to be
   engineering once decided (the ranking's coverage, the fresh card's reason) and both
   shipped the same day; the other four are refusals on the record.
